...
interface DailyDeedsHookApi {
  getToday: { useQuery: (input: { lang: Lang }) => { data: DailyDeedsToday | undefined; isError: boolean } };
  toggle: {
    useMutation: (opts: {
...
interface DailyDeedsUtilsApi {
  getToday: {
    cancel: (input: { lang: Lang }) => Promise<void>;
    getData: (input: { lang: Lang }) => DailyDeedsToday | undefined;
    setData: (
      input: { lang: Lang },
...
export function DailyDeedsCard({ lang, isRTL }: Props) {
  ...
  const todayQuery = dailyDeeds.dailyDeeds.getToday.useQuery({ lang });
  const toggleMutation = dailyDeeds.dailyDeeds.toggle.useMutation({
    onMutate: async ({ deedId, done }) => {
      await utils.dailyDeeds.getToday.cancel({ lang });
      const previous = utils.dailyDeeds.getToday.getData({ lang });
      utils.dailyDeeds.getToday.setData({ lang }, (old) =>
    ...
    onError: (_err, _vars, ctx) => {
      if (ctx?.previous) utils.dailyDeeds.getToday.setData({ lang }, ctx.previous);
    },
