...
import {
  ...
  InsertFamilyMember, familyMembers,
  InsertChild, children,
  InsertChildObservation, childObservations,
  InsertMessage, messages,
  InsertGoalProgress, goalProgress,
  InsertContent, content,
  InsertNewsletter, newsletters,
  InsertNewsletterSubscriber, newsletterSubscribers,
  InsertNewsletterInteraction, newsletterInteractions,
  InsertAdminStat, adminStats,
  InsertAIConversation, aiConversations,
  InsertAIMessage, aiMessages,
  InsertTreatmentPlan, treatmentPlans,
  InsertSpecialistNote, specialistNotes,
  InsertSpecialistAssignment, specialistAssignments,
  InsertAuthor, authors,
  InsertParentChildLink, parentChildLinks,
  InsertSpecialistProfile, specialistProfiles,
  invitationCodes,
  InsertNetworkContact, networkContacts,
  userAuthorizationRoles, InsertUserAuthorizationRole,
  userFunctions, InsertUserFunction,
  contentCategories, contentItems, contentTranslations, contentFiles,
  functionInvitationCodes,
  spouseAdvice, InsertSpouseAdvice,
  translationCache as translationCacheTable,
  partnerships,
  environmentAnalysis, InsertEnvironmentAnalysis,
  childAccounts, InsertChildAccount,
  neighborhoodGroups, InsertNeighborhoodGroup,
  neighborhoodMembers, InsertNeighborhoodMember,
  neighborhoodActivities, InsertNeighborhoodActivity,
  childActivityLog, InsertChildActivityLog,
  childAchievements, InsertChildAchievement,
  childChallenges, InsertChildChallenge,
  peerGroups, InsertPeerGroup,
  peerGroupMembers, InsertPeerGroupMember,
  sharedChildUpdates, InsertSharedChildUpdate,
  familyReminders, InsertFamilyReminder,
  familyActivities, InsertFamilyActivity,
  childDailySummary, InsertChildDailySummary,
  customTasks, InsertCustomTask,
  familyChatMessages, InsertFamilyChatMessage,
  childAiConversations, InsertChildAiConversation,
  childAppUsage, InsertChildAppUsage,
  parentAiConsultations, InsertParentAiConsultation,
} from "../drizzle/schema";
...
export async function updateUserLanguage(userId: number, language: string) {
  const db = await getDb();
  if (!db) return;
  await db.update(users).set({ language, lastActive: new Date() }).where(eq(users.id, userId));
}
...
export async function getUserFamilies(userId: number) {
  ...
  const result = await db.select().from(families).where(sql`${families.id} IN (${sql.join(familyIds.map(id => sql`${id}`), sql`, `)})`);
  return result.map(f => ({
    ...f,
...
export async function joinFamily(familyId: number, userId: number, role: string) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.insert(familyMembers).values({
    familyId,
    ...
    role,
    accepted: true,
    permissions: JSON.stringify({ canEditChildren: role !== "specialist", canViewAdvice: true, canMessage: true, canManageGoals: role !== "specialist" }),
  });
}
...
export async function updateMemberRole(memberId: number, role: string) {
  const db = await getDb();
  if (!db) return;
  await db.update(familyMembers).set({ role }).where(eq(familyMembers.id, memberId));
}
...
export async function getFamilyChildren(familyId: number) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(children).where(and(eq(children.familyId, familyId), isNull(children.deletedAt))).orderBy(children.name);
}

export async function getChildById(childId: number) {
  const db = await getDb();
  if (!db) return undefined;
  const result = await db.select().from(children).where(and(eq(children.id, childId), isNull(children.deletedAt))).limit(1);
  return result.length > 0 ? result[0] : undefined;
}
...
export async function deleteChild(childId: number) {
  ...
  await db.update(children).set({ deletedAt: new Date() }).where(eq(children.id, childId));
}
...
export async function getChildObservations(childId: number, limit = 50) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(childObservations).where(eq(childObservations.childId, childId)).orderBy(desc(childObservations.createdAt)).limit(limit);
}
...
export async function getFamilyMessages(familyId: number, limit = 100) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(messages).where(eq(messages.familyId, familyId)).orderBy(desc(messages.createdAt)).limit(limit);
}
...
export async function getUserMessages(userId: number, familyId: number) {
  ...
  return db.select().from(messages).where(
    and(
      ...
      or(eq(messages.recipientId, userId), isNull(messages.recipientId), eq(messages.senderId, userId))
    )
  ).orderBy(desc(messages.createdAt)).limit(100);
}
 ...
 */
export async function markDirectMessagesRead(recipientId: number, senderId: number) {
  const db = await getDb();
  if (!db) return;
  await db.update(messages).set({ isRead: true, readAt: new Date() }).where(
    and(
      or(
        ...
        and(eq(messages.senderId, senderId), eq(messages.familyId, 0), eq(messages.recipientId, recipientId))
      ),
...
export async function upsertGoalProgress(data: InsertGoalProgress) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.insert(goalProgress).values(data).onDuplicateKeyUpdate({
    set: { status: data.status, notes: data.notes, markedBy: data.markedBy, completedAt: data.status === "completed" ? new Date() : null },
  });
}

export async function getWeekGoalProgress(familyId: number, childId: number, weekId: string) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(goalProgress).where(
    and(eq(goalProgress.familyId, familyId), eq(goalProgress.childId, childId), eq(goalProgress.weekId, weekId))
  );
}
...
export async function getContentList(type?: string, category?: string, limit = 50) {
  const db = await getDb();
  if (!db) return [];
...
export async function getNewsletterSubscribers(active = true) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(newsletterSubscribers).where(eq(newsletterSubscribers.active, active));
}

export async function recordNewsletterInteraction(data: InsertNewsletterInteraction) {
  const db = await getDb();
  if (!db) return;
...
export async function recordStat(type: string, value: number, metadata?: unknown) {
  const db = await getDb();
  if (!db) return;
  const date = new Date().toISOString().split("T")[0];
  await db.insert(adminStats).values({ type, date, value, metadata: metadata ? JSON.stringify(metadata) : null });
}
...
export async function getStats(type?: string, days = 30) {
  ...
  return db.select().from(adminStats).where(and(...conditions)).orderBy(desc(adminStats.date));
}

export async function getDashboardStats() {
  const db = await getDb();
  if (!db) return { totalUsers: 0, totalFamilies: 0, totalChildren: 0, totalMessages: 0, totalConversations: 0 };
...
export async function getSpecialistFamilies(specialistId: number) {
  ...
  const assignments = await db.select().from(specialistAssignments)
    .where(and(eq(specialistAssignments.specialistId, specialistId), eq(specialistAssignments.status, "active")));
  if (assignments.length === 0) return [];
  const familyIds = assignments.map(a => a.familyId);
  const familyList = await db.select().from(families).where(sql`${families.id} IN (${sql.join(familyIds.map(id => sql`${id}`), sql`, `)})`);
...
export async function getSpecialistChildren(specialistId: number) {
  ...
  if (assignments.length === 0) return [];
  const familyIds = assignments.map(a => a.familyId);
  return db.select().from(children).where(sql`${children.familyId} IN (${sql.join(familyIds.map(id => sql`${id}`), sql`, `)})`);
}
...
export async function getSpecialistTreatmentPlans(specialistId: number, status?: string) {
  const db = await getDb();
  if (!db) return [];
  const conditions = [eq(treatmentPlans.specialistId, specialistId)];
  if (status) conditions.push(eq(treatmentPlans.status, status));
  return db.select().from(treatmentPlans).where(and(...conditions)).orderBy(desc(treatmentPlans.updatedAt));
}
...
export async function getChildTreatmentPlans(childId: number) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(treatmentPlans).where(eq(treatmentPlans.childId, childId)).orderBy(desc(treatmentPlans.updatedAt));
}
...
export async function getFamilyTreatmentPlans(familyId: number) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(treatmentPlans).where(eq(treatmentPlans.familyId, familyId)).orderBy(desc(treatmentPlans.updatedAt));
}
...
export async function getTreatmentPlanNotes(treatmentPlanId: number, includePrivate = false) {
  const db = await getDb();
  if (!db) return [];
  const conditions = [eq(specialistNotes.treatmentPlanId, treatmentPlanId)];
  if (!includePrivate) conditions.push(eq(specialistNotes.visibleToParents, true));
  return db.select().from(specialistNotes).where(and(...conditions)).orderBy(desc(specialistNotes.createdAt));
}

/** Create specialist assignment */
export async function createSpecialistAssignment(data: InsertSpecialistAssignment) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function acceptSpecialistAssignment(id: number) {
  const db = await getDb();
  if (!db) return;
  await db.update(specialistAssignments).set({ status: "active", acceptedAt: new Date() }).where(eq(specialistAssignments.id, id));
}
...
export async function getPendingAssignments(specialistId: number) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(specialistAssignments)
    .where(and(eq(specialistAssignments.specialistId, specialistId), eq(specialistAssignments.status, "pending")));
}

/** Get specialist overview stats */
export async function getSpecialistStats(specialistId: number) {
  const db = await getDb();
  if (!db) return { activePlans: 0, totalFamilies: 0, totalChildren: 0, pendingAssignments: 0, totalNotes: 0 };
  const [planCount] = await db.select({ count: sql<number>`count(*)` }).from(treatmentPlans)
    .where(and(eq(treatmentPlans.specialistId, specialistId), eq(treatmentPlans.status, "active")));
  const [familyCount] = await db.select({ count: sql<number>`count(DISTINCT familyId)` }).from(specialistAssignments)
    .where(and(eq(specialistAssignments.specialistId, specialistId), eq(specialistAssignments.status, "active")));
  const [pendingCount] = await db.select({ count: sql<number>`count(*)` }).from(specialistAssignments)
    .where(and(eq(specialistAssignments.specialistId, specialistId), eq(specialistAssignments.status, "pending")));
  const [noteCount] = await db.select({ count: sql<number>`count(*)` }).from(specialistNotes)
...
export async function getAllSpecialists() {
  ...
  return specialists.map(s => ({
    ...s,
    assignmentCount: assignments.filter(a => a.specialistId === s.id && a.status === "active").length,
...
export async function getNetworkContacts(category?: string) {
  ...
  if (!db) return [];
  if (category) {
    return db.select().from(networkContacts).where(eq(networkContacts.category, category as any)).orderBy(desc(networkContacts.createdAt));
  }
  return db.select().from(networkContacts).orderBy(desc(networkContacts.createdAt));
}
...
export async function updateNetworkContact(id: number, data: Partial<InsertNetworkContact>) {
  const db = await getDb();
  if (!db) return;
...
export async function updateUserRole(userId: number, role: string) {
  const db = await getDb();
  if (!db) return;
  await db.update(users).set({ role: role as "user" | "admin" | "super_admin" | "moderator" | "specialist" | "teacher" | "kennisdrager" | "doctor" }).where(eq(users.id, userId));
}
...
export async function getUserAuthRoles(userId: number) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(userAuthorizationRoles).where(eq(userAuthorizationRoles.userId, userId));
}

export async function addUserAuthRole(userId: number, role: string, assignedBy?: number) {
  const db = await getDb();
  if (!db) return;
  const existing = await db.select().from(userAuthorizationRoles)
    .where(and(eq(userAuthorizationRoles.userId, userId), eq(userAuthorizationRoles.role, role as any)))
    .limit(1);
  if (existing.length > 0) return;
  await db.insert(userAuthorizationRoles).values({ userId, role: role as any, assignedBy });
}

export async function removeUserAuthRole(userId: number, role: string) {
  const db = await getDb();
  if (!db) return;
  await db.delete(userAuthorizationRoles)
    .where(and(eq(userAuthorizationRoles.userId, userId), eq(userAuthorizationRoles.role, role as any)));
}
...
export async function getUserFunctions(userId: number) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(userFunctions).where(eq(userFunctions.userId, userId));
}

export async function addUserFunction(userId: number, functionRole: string, specialization?: string, city?: string, assignedBy?: number) {
  const db = await getDb();
  if (!db) return;
  const existing = await db.select().from(userFunctions)
    .where(and(eq(userFunctions.userId, userId), eq(userFunctions.functionRole, functionRole as any)))
    .limit(1);
  if (existing.length > 0) return;
  await db.insert(userFunctions).values({ userId, functionRole: functionRole as any, specialization, city, assignedBy });
}

export async function removeUserFunction(userId: number, functionRole: string) {
  const db = await getDb();
  if (!db) return;
  await db.delete(userFunctions)
    .where(and(eq(userFunctions.userId, userId), eq(userFunctions.functionRole, functionRole as any)));
}
...
export async function userHasAuthRole(userId: number, role: string): Promise<boolean> {
  const db = await getDb();
  if (!db) return false;
  const result = await db.select().from(userAuthorizationRoles)
    .where(and(eq(userAuthorizationRoles.userId, userId), eq(userAuthorizationRoles.role, role as any)))
    .limit(1);
...
export async function userIsAdmin(userId: number): Promise<boolean> {
  const db = await getDb();
  if (!db) return false;
  const roles = await db.select().from(userAuthorizationRoles)
    .where(eq(userAuthorizationRoles.userId, userId));
  return roles.some(r => ["super_admin", "admin", "moderator"].includes(r.role));
}
...
export async function getRegistrationAnalytics(days: number = 30) {
  ...
  }).from(users).where(sql`${users.createdAt} >= ${since}`).groupBy(sql`DATE(${users.createdAt})`).orderBy(sql`DATE(${users.createdAt})`);
  return results;
}
...
export async function getActiveUsersAnalytics(days: number = 30) {
  ...
  const results = await db.select({
    ...
    count: sql<number>`count(*)`,
  }).from(users).where(sql`${users.lastActive} >= ${since}`).groupBy(sql`DATE(${users.lastActive})`).orderBy(sql`DATE(${users.lastActive})`);
  return results;
}
...
export async function getChildrenByAgeGroup() {
  ...
  if (!db) return [];
  const allChildren = await db.select().from(children);
  const ageGroups: Record<string, number> = { "0-2": 0, "3-5": 0, "5-7": 0, "7-10": 0, "10-12": 0, "12-16": 0, "16+": 0 };
  allChildren.forEach(c => {
    if (!c.birthDate) return;
    const age = (Date.now() - new Date(c.birthDate).getTime()) / (365.25 * 86400000);
...
export async function getPublishedArticles(category?: string, limit = 20) {
  ...
  if (category) conditions.push(eq(content.category, category));
  return db.select().from(content).where(and(...conditions)).orderBy(desc(content.updatedAt)).limit(limit);
}

export async function getArticleBySlug(slug: string) {
  const db = await getDb();
  if (!db) return undefined;
  const result = await db.select().from(content).where(and(eq(content.slug, slug), eq(content.type, "article"))).limit(1);
  return result.length > 0 ? result[0] : undefined;
}
...
export async function searchArticles(query: string, limit = 20) {
  ...
  return db.select().from(content).where(
    and(
      eq(content.type, "article"),
  ...
  ).orderBy(desc(content.updatedAt)).limit(limit);
}
...
export async function getArticlesByAuthor(authorId: number, limit = 20) {
  ...
  if (!db) return [];
  return db.select().from(content).where(
    and(eq(content.type, "article"), eq(content.published, true), eq(content.authorId, authorId))
  ).orderBy(desc(content.updatedAt)).limit(limit);
}
 ...
 */
export async function generateUserPublicId(userId: number, birthDate: string): Promise<string> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  ...
  await db.update(users).set({ publicId, birthDate }).where(eq(users.id, userId));
  return publicId;
}
 ...
 */
export async function generateChildPublicId(childId: number, birthDate: string): Promise<string> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const datePart = birthDate.replace(/-/g, "");
  const dayLetters = getDayLetters(birthDate);
 ...
 */
export async function setUserBirthDateAndGenerateId(userId: number, birthDate: string): Promise<string> {
...
export async function getUserByPublicId(publicId: string) {
  ...
  let result = await db.select().from(users).where(eq(users.publicId, publicId)).limit(1);
  if (result.length > 0) return result[0];
  // Try case-insensitive match (UPPER comparison)
  const upper = publicId.toUpperCase();
  result = await db.select().from(users).where(sql`UPPER(${users.publicId}) = ${upper}`).limit(1);
  if (result.length > 0) return result[0];
  ...
  if (cleaned !== upper) {
    result = await db.select().from(users).where(sql`UPPER(${users.publicId}) = ${cleaned}`).limit(1);
    if (result.length > 0) return result[0];
  }
  return undefined;
}
...
export async function getChildByPublicId(publicId: string) {
  ...
  if (!db) return undefined;
  // Exact match first
  let result = await db.select().from(children).where(eq(children.publicId, publicId)).limit(1);
  if (result.length > 0) return result[0];
  // Case-insensitive match
  const upper = publicId.toUpperCase();
  result = await db.select().from(children).where(sql`UPPER(${children.publicId}) = ${upper}`).limit(1);
  if (result.length > 0) return result[0];
  ...
  if (parts.length === 3) {
    const datePart = parts[0];
    const seqPart = parts[2];
    result = await db.select().from(children).where(
      sql`${children.publicId} LIKE ${datePart + '_%_' + seqPart}`
    ).limit(1);
    if (result.length > 0) return result[0];
  }
  return undefined;
...
export async function linkParentToChild(data: {
  ...
  const existing = await db.select().from(parentChildLinks)
    .where(and(eq(parentChildLinks.parentId, data.parentId), eq(parentChildLinks.childId, data.childId)))
    .limit(1);
...
export async function getLinkedChildren(parentId: number) {
  ...
  const childList = await db.select().from(children)
    .where(sql`${children.id} IN (${sql.join(childIds.map(id => sql`${id}`), sql`, `)}) AND ${children.deletedAt} IS NULL`);
...
export async function getLinkedParents(childId: number) {
  ...
  const parentList = await db.select().from(users)
...
export async function confirmParentChildLink(linkId: number) {
  const db = await getDb();
  if (!db) return;
  await db.update(parentChildLinks).set({ confirmed: true }).where(eq(parentChildLinks.id, linkId));
}
 ...
 */
export async function confirmAllLinksFromSender(senderId: number, confirmingUserId: number) {
  const db = await getDb();
  if (!db) return;
 ...
 */
export async function removeAllLinksFromSender(senderId: number, rejectingUserId: number) {
  const db = await getDb();
  if (!db) return;
 ...
 */
export async function linkChildByPublicId(childPublicId: string, parentId: number, relationship: string) {
  const db = await getDb();
  if (!db) return null;
  ...
  const existingLink = await linkDb.select().from(parentChildLinks)
    .where(and(eq(parentChildLinks.parentId, parentId), eq(parentChildLinks.childId, child.id)))
    .limit(1);
...
export async function getCoParents(userId: number) {
  ...
  const coParentMap = new Map<number, { relationship: string; sharedChildIds: number[] }>();
  
  // Source 1: partnerships table (always persists)
  try {
    const partnershipRows = await db.select().from(partnerships)
      .where(and(
    ...
    for (const p of partnershipRows) {
      ...
      coParentMap.set(partnerId, { relationship: "partner", sharedChildIds: [] });
    }
  } catch (e) {
    console.warn("[getCoParents] partnerships query failed (table may not exist yet):", e);
  }
  
  // Source 2: parentChildLinks (shared children)
  const myLinks = await db.select().from(parentChildLinks)
    .where(eq(parentChildLinks.parentId, userId));
  const myChildIds = myLinks.map(l => l.childId);
  
  if (myChildIds.length > 0) {
    const otherLinks = await db.select().from(parentChildLinks)
      .where(sql`${parentChildLinks.childId} IN (${sql.join(myChildIds.map(id => sql`${id}`), sql`, `)}) AND ${parentChildLinks.parentId} != ${userId}`);
    ...
    for (const link of otherLinks) {
      ...
      if (existing) {
        ...
        coParentMap.set(link.parentId, { relationship: link.relationship || "parent", sharedChildIds: [link.childId] });
      }
    }
  }
  ...
  const parentList = await db.select().from(users)
    .where(sql`${users.id} IN (${sql.join(otherParentIds.map(id => sql`${id}`), sql`, `)})`);
  
  // Get child details for shared children
  const allSharedChildIds = Array.from(new Set(Array.from(coParentMap.values()).flatMap(v => v.sharedChildIds)));
  let childList: any[] = [];
  if (allSharedChildIds.length > 0) {
    childList = await db.select().from(children)
      .where(sql`${children.id} IN (${sql.join(allSharedChildIds.map(id => sql`${id}`), sql`, `)})`);
  }
  
  // Build co-parent list
  return parentList.map(parent => {
    const info = coParentMap.get(parent.id)!;
    const sharedChildren = childList.filter(c => info.sharedChildIds.includes(c.id));
    const profileData = typeof parent.profileData === "string" ? JSON.parse(parent.profileData as string) : (parent.profileData || {});
    const gender = (profileData as any)?.parentProfile?.gender || (profileData as any)?.gender || null;
    return {
      id: parent.id,
      name: parent.name,
      ...
      sharedChildren: sharedChildren.map(c => ({ id: c.id, name: c.name, publicId: c.publicId })),
    };
  });
}
...
export async function upsertSpecialistProfile(userId: number, data: Partial<InsertSpecialistProfile>) {
  const db = await getDb();
  if (!db) return null;
  const existing = await db.select().from(specialistProfiles).where(eq(specialistProfiles.userId, userId));
  if (existing.length > 0) {
    await db.update(specialistProfiles).set({ ...data, updatedAt: new Date() }).where(eq(specialistProfiles.userId, userId));
    return existing[0].id;
  } else {
    const result = await db.insert(specialistProfiles).values({ ...data, userId } as InsertSpecialistProfile);
...
export async function getSpecialistProfile(userId: number) {
  const db = await getDb();
  if (!db) return null;
  const rows = await db.select().from(specialistProfiles).where(eq(specialistProfiles.userId, userId));
  return rows[0] ?? null;
}
...
export async function getAvailableSpecialists() {
  const db = await getDb();
  if (!db) return [];
  const profiles = await db.select().from(specialistProfiles).where(eq(specialistProfiles.isAvailable, true));
  // Get user info for each specialist
  const result = [];
  for (const profile of profiles) {
    const userRows = await db.select().from(users).where(eq(users.id, profile.userId));
    if (userRows.length > 0) {
      result.push({ ...profile, user: { id: userRows[0].id, name: userRows[0].name, email: userRows[0].email } });
    }
  }
  return result;
}
...
export async function findNearestSpecialist(lat: number, lon: number, excludeIds: number[] = []) {
  const db = await getDb();
  if (!db) return [];
  const profiles = await db.select().from(specialistProfiles).where(eq(specialistProfiles.isAvailable, true));
...
export async function findSpecialistsByCity(city: string) {
  const db = await getDb();
  if (!db) return [];
  const profiles = await db.select().from(specialistProfiles).where(
    and(eq(specialistProfiles.isAvailable, true), like(specialistProfiles.city, `%${city}%`))
  );
  const result = [];
  for (const profile of profiles) {
    const userRows = await db.select().from(users).where(eq(users.id, profile.userId));
...
export async function findSpecialistsByCountry(country: string) {
  ...
  if (!db) return [];
  const profiles = await db.select().from(specialistProfiles).where(
    and(eq(specialistProfiles.isAvailable, true), like(specialistProfiles.country, `%${country}%`))
  );
  const result = [];
  for (const profile of profiles) {
    const userRows = await db.select().from(users).where(eq(users.id, profile.userId));
...
export async function getFallbackPhoneNumbers() {
  ...
  if (!db) return [];
  const profiles = await db.select().from(specialistProfiles).where(
    and(eq(specialistProfiles.isAvailable, true), sql`${specialistProfiles.phone} IS NOT NULL AND ${specialistProfiles.phone} != ''`)
  );
  const result = [];
  for (const profile of profiles) {
    const userRows = await db.select().from(users).where(eq(users.id, profile.userId));
    if (userRows.length > 0) {
      result.push({ name: profile.displayName || userRows[0].name, phone: profile.phone, city: profile.city, country: profile.country, expertise: profile.expertise });
    }
  }
  return result;
}
...
export async function updateSpecialistOnlineStatus(userId: number) {
  const db = await getDb();
  if (!db) return;
  await db.update(specialistProfiles).set({ lastOnline: new Date() }).where(eq(specialistProfiles.userId, userId));
}
...
export async function getSpecialistFamilyAnalysis(specialistId: number) {
  ...
  const assignments = await db.select().from(specialistAssignments).where(
    and(eq(specialistAssignments.specialistId, specialistId), eq(specialistAssignments.status, "active"))
  );
  
  const result = [];
  for (const assignment of assignments) {
    // Get family info
    const familyRows = await db.select().from(families).where(eq(families.id, assignment.familyId));
    if (familyRows.length === 0) continue;
    
    // Get family members
    const members = await db.select().from(familyMembers).where(eq(familyMembers.familyId, assignment.familyId));
    
    // Get children
    const familyChildren = await db.select().from(children).where(eq(children.familyId, assignment.familyId));
    
    // Get parent user profiles
    const parentProfiles = [];
    for (const member of members) {
      const userRows = await db.select().from(users).where(eq(users.id, member.userId));
      if (userRows.length > 0) {
        parentProfiles.push({
          id: userRows[0].id,
    ...
    for (const child of familyChildren) {
      const obs = await db.select().from(childObservations).where(eq(childObservations.childId, child.id));
      allObservations.push(...obs.map(o => ({ ...o, childName: child.name })));
    }
    ...
    result.push({
      ...
      observations: allObservations.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).slice(0, 20),
    });
  }
...
export async function updateUserPushToken(userId: number, pushToken: string | null) {
  const db = await getDb();
  if (!db) return;
  await db.update(users).set({ pushToken } as any).where(eq(users.id, userId));
}

export async function getUserPushToken(userId: number): Promise<string | null> {
  const db = await getDb();
  if (!db) return null;
  const result = await db.select({ pushToken: sql<string>`pushToken` }).from(users).where(eq(users.id, userId)).limit(1);
...
export async function validateInvitationCode(code: string) {
  const db = await getDb();
  if (!db) return null;
  const result = await db.select().from(invitationCodes).where(
    and(
...
export async function generateInvitationCode(createdBy?: number): Promise<string> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
 ...
 */
export async function getUserLanguage(userId: number): Promise<"nl" | "en" | "ar"> {
  const db = await getDb();
  if (!db) return "nl";
  const result = await db.select({ language: users.language }).from(users).where(eq(users.id, userId)).limit(1);
...
export async function sendLocalizedPush(
  ...
  titleNl: string, titleEn: string, titleAr: string,
  bodyNl: string, bodyEn: string, bodyAr: string,
...
export async function sendPushNotification(
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(`[Push] Failed to send to user ${recipientUserId}: ${response.status}`);
      return false;
    }
    return true;
  } catch (error) {
...
export async function getRecentMessages(limit: number = 100) {
  const db = await getDb();
  if (!db) return [];
  const result = await db.select().from(messages).orderBy(desc(messages.createdAt)).limit(limit);
  return result;
}
...
export async function getAllContentCategories() {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(contentCategories).orderBy(contentCategories.sortOrder);
}
...
export async function createContentCategory(data: { slug: string; nameNl: string; nameEn: string; nameAr: string; appSection: string; ageGroup?: string; sortOrder?: number }) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function getAllContentItems(opts?: { status?: string; categoryId?: number; contentType?: string }) {
  const db = await getDb();
  if (!db) return [];
  let query = db.select().from(contentItems);
  const conditions: any[] = [];
  if (opts?.status) conditions.push(eq(contentItems.status, opts.status as any));
  if (opts?.categoryId) conditions.push(eq(contentItems.categoryId, opts.categoryId));
  if (opts?.contentType) conditions.push(eq(contentItems.contentType, opts.contentType as any));
  if (conditions.length > 0) {
...
export async function createContentItem(data: { categoryId?: number; contentType: string; status?: string; originalLanguage?: string; tags?: string; authorId?: number; mediaUrl?: string }) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function updateContentItem(id: number, data: Partial<{ categoryId: number; contentType: string; status: string; tags: string; mediaUrl: string; publishedAt: Date }>) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.update(contentItems).set({ ...data, updatedAt: new Date() } as any).where(eq(contentItems.id, id));
}

export async function deleteContentItem(id: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.delete(contentTranslations).where(eq(contentTranslations.contentId, id));
...
export async function getContentTranslations(contentId: number) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(contentTranslations).where(eq(contentTranslations.contentId, contentId));
}

export async function upsertContentTranslation(contentId: number, language: string, data: { title: string; summary?: string; body?: string; isAutoTranslated?: boolean }) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const existing = await db.select().from(contentTranslations)
    .where(and(eq(contentTranslations.contentId, contentId), eq(contentTranslations.language, language as any)));
  if (existing.length > 0) {
    await db.update(contentTranslations).set({ ...data, updatedAt: new Date() } as any)
      .where(and(eq(contentTranslations.contentId, contentId), eq(contentTranslations.language, language as any)));
  } else {
    await db.insert(contentTranslations).values({ contentId, language, ...data } as any);
  }
}
...
export async function getContentFiles(contentId: number) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(contentFiles).where(eq(contentFiles.contentId, contentId));
}

export async function addContentFile(data: { contentId: number; fileName: string; fileType: string; filePath: string; fileSize?: number; language?: string }) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function getPublishedContent(opts?: { section?: string; categoryId?: number; language?: string }) {
  const db = await getDb();
  if (!db) return [];
  ...
  for (const item of filteredItems) {
    ...
    const translation = translations.find((t: any) => t.language === lang) || translations[0];
    if (translation) {
...
export async function getPublishedContentBySection(section: string, language: string = 'nl') {
  const db = await getDb();
  if (!db) return [];
  ...
  for (const item of filtered) {
    const translations = await db.select().from(contentTranslations)
      .where(eq(contentTranslations.contentId, item.id));
    const translation = translations.find((t: any) => t.language === language) || translations.find((t: any) => t.language === 'nl') || translations[0];
...
export async function getCmsContentBySection(appSection: string, language: string = 'nl', contentType?: string, limit: number = 50) {
  const db = await getDb();
  if (!db) return [];
  const categories = await db.select().from(contentCategories)
  ...
  if (contentType) filtered = filtered.filter((i: any) => i.contentType === contentType);
  ...
  for (const item of filtered) {
    ...
    const category = categories.find((c: any) => c.id === item.categoryId);
    if (translation) {
      results.push({ id: item.id, contentType: item.contentType, mediaUrl: item.mediaUrl, tags: item.tags, title: translation.title, summary: translation.summary, body: translation.body, language: translation.language, category: category ? { id: category.id, nameNl: category.nameNl, nameEn: category.nameEn, nameAr: category.nameAr } : null, createdAt: item.createdAt });
    }
  }
  return results;
}
...
export async function getCmsContentItemWithTranslations(id: number) {
  ...
  const files = await db.select().from(contentFiles)
    .where(eq(contentFiles.contentId, id));
  const category = item.categoryId ? (await db.select().from(contentCategories).where(eq(contentCategories.id, item.categoryId)))[0] : null;
...
export async function getAllInvitationCodes() {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(functionInvitationCodes).orderBy(functionInvitationCodes.createdAt);
}

export async function createInvitationCode(data: { code: string; functionRole: string; restrictedEmail?: string; maxUses?: number; createdBy?: number; expiresAt?: Date }) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function useInvitationCode(code: string, userId: number): Promise<{ success: boolean; functionRole?: string; error?: string }> {
  const db = await getDb();
  if (!db) return { success: false, error: "Database not available" };
  
  const rows = await db.select().from(functionInvitationCodes)
    .where(eq(functionInvitationCodes.code, code));
  if (rows.length === 0) return { success: false, error: "Code niet gevonden" };
  
  const invite = rows[0];
  if (!invite.isActive) return { success: false, error: "Code is niet meer actief" };
  if (invite.maxUses && invite.usedCount >= invite.maxUses) return { success: false, error: "Code is al maximaal gebruikt" };
  if (invite.expiresAt && new Date(invite.expiresAt) < new Date()) return { success: false, error: "Code is verlopen" };
  
  // Assign the function
  await addUserFunction(userId, invite.functionRole);
  
  // Increment usage
  await db.update(functionInvitationCodes)
...
export async function deactivateInvitationCode(id: number) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.update(functionInvitationCodes).set({ isActive: false } as any).where(eq(functionInvitationCodes.id, id));
}
...
export async function broadcastLocalizedPush(
  ...
  }).from(users).where(sql`pushToken IS NOT NULL AND pushToken != ''`);
...
export async function getSpouseAdviceForUser(recipientId: number, limit = 20) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(spouseAdvice)
...
export async function markSpouseAdviceRead(adviceId: number) {
  const db = await getDb();
  if (!db) return;
  await db.update(spouseAdvice).set({ isRead: true }).where(eq(spouseAdvice.id, adviceId));
}

export async function markSpouseAdviceHelpful(adviceId: number, helpful: boolean) {
  const db = await getDb();
  if (!db) return;
  await db.update(spouseAdvice).set({ isHelpful: helpful }).where(eq(spouseAdvice.id, adviceId));
}
 ...
 */
export async function getPartnerOfUser(userId: number): Promise<{ id: number; name: string | null; profileData: any } | null> {
  const db = await getDb();
  if (!db) return null;
  ...
  if (partnershipRows.length > 0) {
    ...
    const partnerId = p.userId1 === userId ? p.userId2 : p.userId1;
    const partner = await db.select().from(users).where(and(eq(users.id, partnerId), isNull(users.deletedAt))).limit(1);
    if (partner.length > 0) {
      return { id: partner[0].id, name: partner[0].name, profileData: partner[0].profileData };
    }
  }
  
  // 2. Fallback: detect via shared children (legacy)
  const myLinks = await db.select().from(parentChildLinks)
    .where(and(eq(parentChildLinks.parentId, userId), eq(parentChildLinks.confirmed, true)));
  if (myLinks.length === 0) return null;
  const myChildIds = myLinks.map(l => l.childId);
  for (const childId of myChildIds) {
    const otherLinks = await db.select().from(parentChildLinks)
      .where(and(eq(parentChildLinks.childId, childId), eq(parentChildLinks.confirmed, true)));
    const partnerId = otherLinks.find(l => l.parentId !== userId)?.parentId;
    if (partnerId) {
      const partner = await db.select().from(users).where(and(eq(users.id, partnerId), isNull(users.deletedAt))).limit(1);
      if (partner.length > 0) {
        // Auto-create partnership record for persistence
        await createPartnership(userId, partnerId, userId);
        return { id: partner[0].id, name: partner[0].name, profileData: partner[0].profileData };
      }
    }
  }
...
export async function createPartnership(userId1: number, userId2: number, initiatedBy: number) {
  const db = await getDb();
  if (!db) return null;
  // Check if already exists
  const existing = await db.select().from(partnerships)
    .where(and(
      or(
  ...
  // Create new
  const [result] = await db.insert(partnerships).values({ userId1, userId2, initiatedBy });
  return { id: result.insertId, userId1, userId2, status: "active", initiatedBy };
}
 ...
 */
export async function getSpouseInteractionData(userId: number, partnerId: number) {
  const db = await getDb();
  if (!db) return { goals: [], conversations: [], messages: [], profileData: null, childrenData: [] };
  // Get recent goal progress for this user
  const goals = await db.select().from(goalProgress)
  ...
  const userRow = await db.select({ profileData: users.profileData }).from(users).where(eq(users.id, userId)).limit(1);
  const profileData = userRow[0]?.profileData || null;
  // Get children data with environment analysis (shared children)
  const userFamilyRows = await db.select({ familyId: familyMembers.familyId }).from(familyMembers).where(eq(familyMembers.userId, userId));
...
function hashText(text: string): string {
  ...
  return hex + lenHash.toString(16).padStart(8, '0') + text.substring(0, 48).replace(/[^a-zA-Z0-9\u0600-\u06FF]/g, '').substring(0, 48);
}
 ...
 */
export async function getCachedTranslations(texts: string[], targetLang: "nl" | "en"): Promise<Map<string, string>> {
  const db = await getDb();
  ...
  for (const hash of hashes) {
    const rows = await db.select().from(translationCacheTable)
      .where(and(
...
export async function saveTranslationsToCache(
  entries: { sourceText: string; translatedText: string; targetLang: "nl" | "en"; category?: string }[]
): Promise<void> {
  const db = await getDb();
  ...
  for (const entry of entries) {
    ...
    // Check if already exists
    const existing = await db.select({ id: translationCacheTable.id }).from(translationCacheTable)
      .where(and(
        eq(translationCacheTable.sourceHash, hash),
...
export async function createEnvironmentAnalysis(data: InsertEnvironmentAnalysis) {
  const database = await getDb();
  if (!database) return null;
  const [result] = await database.insert(environmentAnalysis).values(data).$returningId();
  return result;
}
...
export async function getEnvironmentAnalyses(userId: number, childId?: number) {
  ...
  return database.select().from(environmentAnalysis).where(and(...conditions)).orderBy(desc(environmentAnalysis.analyzedAt));
}

export async function getLatestEnvironmentAnalysis(userId: number, childId: number) {
  const database = await getDb();
  if (!database) return null;
  const results = await database.select().from(environmentAnalysis)
    .where(and(eq(environmentAnalysis.userId, userId), eq(environmentAnalysis.childId, childId)))
...
export async function createChildAccount(data: InsertChildAccount) {
  const database = await getDb();
  if (!database) return null;
  const [result] = await database.insert(childAccounts).values(data).$returningId();
  return result;
}

export async function getChildAccountByAccessCode(parentId: number, accessCode: string) {
  const database = await getDb();
  if (!database) return null;
  const results = await database.select().from(childAccounts)
    .where(and(
...
export async function getChildAccountsByParent(parentId: number) {
  const database = await getDb();
  if (!database) return [];
  return database.select().from(childAccounts)
    .where(and(eq(childAccounts.parentId, parentId), eq(childAccounts.isActive, true)));
}

export async function updateChildAccountLastActive(accountId: number) {
  const database = await getDb();
  if (!database) return;
  await database.update(childAccounts).set({ lastActive: new Date() }).where(eq(childAccounts.id, accountId));
}
...
export async function createNeighborhoodGroup(data: InsertNeighborhoodGroup) {
  const database = await getDb();
  if (!database) return null;
  const [result] = await database.insert(neighborhoodGroups).values(data).$returningId();
  return result;
}

export async function getNeighborhoodGroupsNearby(lat: number, lng: number, radiusKm: number = 5) {
  const database = await getDb();
  if (!database) return [];
  ...
  return database.select().from(neighborhoodGroups)
    .where(and(
...
export async function addNeighborhoodMember(data: InsertNeighborhoodMember) {
  const database = await getDb();
  if (!database) return null;
  const [result] = await database.insert(neighborhoodMembers).values(data).$returningId();
  return result;
}
...
export async function getNeighborhoodMembers(groupId: number) {
  const database = await getDb();
  if (!database) return [];
  return database.select().from(neighborhoodMembers)
...
export async function createChildChallenge(data: InsertChildChallenge) {
  const database = await getDb();
  if (!database) return null;
  const [result] = await database.insert(childChallenges).values(data).$returningId();
  return result;
}

export async function getChildChallenges(childAccountId: number, date?: string) {
  const database = await getDb();
  if (!database) return [];
  ...
  return database.select().from(childChallenges).where(and(...conditions)).orderBy(desc(childChallenges.challengeDate));
}

export async function completeChildChallenge(challengeId: number) {
  const database = await getDb();
  if (!database) return;
  await database.update(childChallenges).set({ status: "completed", completedAt: new Date() }).where(eq(childChallenges.id, challengeId));
}
...
export async function createChildAchievement(data: InsertChildAchievement) {
  const database = await getDb();
  if (!database) return null;
  const [result] = await database.insert(childAchievements).values(data).$returningId();
  return result;
}
...
export async function getChildAchievements(childAccountId: number) {
  const database = await getDb();
  if (!database) return [];
  return database.select().from(childAchievements)
...
export async function logChildActivity(data: InsertChildActivityLog) {
  const database = await getDb();
  if (!database) return null;
  const [result] = await database.insert(childActivityLog).values(data).$returningId();
  return result;
}

export async function getChildActivityLog(childAccountId: number, limit: number = 50) {
  const database = await getDb();
  if (!database) return [];
  return database.select().from(childActivityLog)
...
export async function createSharedChildUpdate(data: InsertSharedChildUpdate) {
  const database = await getDb();
  if (!database) return null;
  const [result] = await database.insert(sharedChildUpdates).values(data).$returningId();
  return result;
}

export async function getSharedChildUpdates(childId: number, limit: number = 30) {
  const database = await getDb();
  if (!database) return [];
  return database.select().from(sharedChildUpdates)
...
export async function markSharedUpdateRead(updateId: number) {
  const database = await getDb();
  if (!database) return;
  await database.update(sharedChildUpdates).set({ isRead: true, readAt: new Date() }).where(eq(sharedChildUpdates.id, updateId));
}
...
export async function createFamilyReminder(data: InsertFamilyReminder) {
  const database = await getDb();
  if (!database) return null;
  const [result] = await database.insert(familyReminders).values(data).$returningId();
  return result;
}
...
export async function getFamilyReminders(familyId: number) {
  const database = await getDb();
  if (!database) return [];
  return database.select().from(familyReminders)
    .where(and(eq(familyReminders.familyId, familyId), eq(familyReminders.isActive, true)))
...
export async function createFamilyActivity(data: InsertFamilyActivity) {
  const database = await getDb();
  if (!database) return null;
  const [result] = await database.insert(familyActivities).values(data).$returningId();
  return result;
}
...
export async function getFamilyActivities(familyId: number) {
  const database = await getDb();
  if (!database) return [];
  return database.select().from(familyActivities)
    .where(eq(familyActivities.familyId, familyId))
    .orderBy(desc(familyActivities.createdAt));
}

export async function voteFamilyActivity(activityId: number, userId: number, vote: string) {
  const database = await getDb();
  if (!database) return;
  const [activity] = await database.select().from(familyActivities).where(eq(familyActivities.id, activityId));
  if (!activity) return;
  const currentVotes = (activity.votes as Record<string, string>) || {};
  currentVotes[String(userId)] = vote;
  await database.update(familyActivities).set({ votes: currentVotes }).where(eq(familyActivities.id, activityId));
}
...
export async function createNeighborhoodActivity(data: InsertNeighborhoodActivity) {
  const database = await getDb();
  if (!database) return null;
  const [result] = await database.insert(neighborhoodActivities).values(data).$returningId();
  return result;
}
...
export async function getNeighborhoodActivities(groupId: number) {
  const database = await getDb();
  if (!database) return [];
  return database.select().from(neighborhoodActivities)
...
export async function createPeerGroup(data: InsertPeerGroup) {
  const database = await getDb();
  if (!database) return null;
  const [result] = await database.insert(peerGroups).values(data).$returningId();
  return result;
}
...
export async function getPeerGroupByCode(inviteCode: string) {
  const database = await getDb();
  if (!database) return null;
  const results = await database.select().from(peerGroups)
    .where(and(eq(peerGroups.inviteCode, inviteCode), eq(peerGroups.isActive, true)))
    .limit(1);
  return results[0] || null;
}
...
export async function addPeerGroupMember(data: InsertPeerGroupMember) {
  const database = await getDb();
  if (!database) return null;
  const [result] = await database.insert(peerGroupMembers).values(data).$returningId();
  return result;
}
...
export async function getPeerGroupMembers(groupId: number) {
  const database = await getDb();
  if (!database) return [];
  return database.select().from(peerGroupMembers)
    .where(eq(peerGroupMembers.groupId, groupId));
}

export async function approvePeerGroupMember(memberId: number, parentId: number) {
  const database = await getDb();
  if (!database) return;
  await database.update(peerGroupMembers)
...
export async function createCustomTask(data: InsertCustomTask) {
  const database = await getDb();
  if (!database) return null;
  const [result] = await database.insert(customTasks).values(data).$returningId();
  return result;
}
export async function getCustomTasks(childAccountId: number, status?: string) {
  ...
  if (!database) return [];
  if (status) {
    return database.select().from(customTasks)
      .where(and(eq(customTasks.childAccountId, childAccountId), eq(customTasks.status, status)))
...
export async function updateCustomTask(taskId: number, data: Partial<InsertCustomTask>) {
  const database = await getDb();
  if (!database) return;
  await database.update(customTasks).set(data).where(eq(customTasks.id, taskId));
}
...
export async function completeCustomTask(taskId: number, childNote?: string, proofImageUrl?: string) {
  const database = await getDb();
  if (!database) return;
  await database.update(customTasks).set({
    status: "completed",
...
export async function sendFamilyChatMessage(data: InsertFamilyChatMessage) {
  const database = await getDb();
  if (!database) return null;
  const [result] = await database.insert(familyChatMessages).values(data).$returningId();
  return result;
}
export async function getFamilyChatMessages(parentId: number, childAccountId: number, limit: number = 100) {
  const database = await getDb();
  if (!database) return [];
  return database.select().from(familyChatMessages)
    .where(and(
...
export async function markFamilyChatRead(parentId: number, childAccountId: number, readerType: string) {
  const database = await getDb();
  if (!database) return;
  const senderType = readerType === "parent" ? "child" : "parent";
  await database.update(familyChatMessages)
...
export async function getUnreadChatCount(parentId: number, childAccountId: number, readerType: string) {
  const database = await getDb();
  if (!database) return 0;
  const senderType = readerType === "parent" ? "child" : "parent";
  const rows = await database.select({ count: sql<number>`count(*)` }).from(familyChatMessages)
    .where(and(
      eq(familyChatMessages.parentId, parentId),
      eq(familyChatMessages.childAccountId, childAccountId),
      eq(familyChatMessages.senderType, senderType),
...
// --- Child Daily Summary ---
export async function getChildDailySummary(childAccountId: number, date: string) {
  const database = await getDb();
  if (!database) return null;
  const rows = await database.select().from(childDailySummary)
    .where(and(eq(childDailySummary.childAccountId, childAccountId), eq(childDailySummary.date, date)));
  return rows[0] || null;
}
export async function getChildWeeklySummary(childAccountId: number, startDate: string, endDate: string) {
  const database = await getDb();
  if (!database) return [];
  return database.select().from(childDailySummary)
    .where(and(
...
export async function upsertChildDailySummary(childAccountId: number, date: string, data: Partial<InsertChildDailySummary>) {
  const database = await getDb();
  if (!database) return;
  const existing = await getChildDailySummary(childAccountId, date);
  if (existing) {
    await database.update(childDailySummary).set(data).where(eq(childDailySummary.id, existing.id));
  } else {
    await database.insert(childDailySummary).values({ childAccountId, date, ...data });
  }
}

// --- Child AI Conversations ---
export async function createChildAiConversation(data: InsertChildAiConversation) {
  const database = await getDb();
  if (!database) return null;
  const [result] = await database.insert(childAiConversations).values(data).$returningId();
  return result;
}
export async function getChildAiConversations(childAccountId: number, limit: number = 20) {
  const database = await getDb();
  if (!database) return [];
  return database.select().from(childAiConversations)
...
export async function getChildAiConversation(conversationId: number) {
  const database = await getDb();
  if (!database) return null;
  const rows = await database.select().from(childAiConversations)
    .where(eq(childAiConversations.id, conversationId));
  return rows[0] || null;
}
export async function updateChildAiConversation(conversationId: number, data: Partial<InsertChildAiConversation>) {
  const database = await getDb();
  if (!database) return;
  await database.update(childAiConversations).set(data).where(eq(childAiConversations.id, conversationId));
}
...
export async function logChildAppUsage(data: InsertChildAppUsage) {
  const database = await getDb();
  if (!database) return null;
  const [result] = await database.insert(childAppUsage).values(data).$returningId();
  return result;
}
export async function getChildAppUsage(childAccountId: number, date: string) {
  const database = await getDb();
  if (!database) return [];
  return database.select().from(childAppUsage)
    .where(and(eq(childAppUsage.childAccountId, childAccountId), eq(childAppUsage.date, date)))
    .orderBy(desc(childAppUsage.usageSeconds));
}
export async function getChildAppUsageRange(childAccountId: number, startDate: string, endDate: string) {
  const database = await getDb();
  if (!database) return [];
  return database.select().from(childAppUsage)
    .where(and(
...
export async function createParentAiConsultation(data: InsertParentAiConsultation) {
  const database = await getDb();
  if (!database) return null;
  const [result] = await database.insert(parentAiConsultations).values(data).$returningId();
  return result;
}
export async function getParentAiConsultations(parentId: number, consultationType?: string) {
  const database = await getDb();
  if (!database) return [];
  if (consultationType) {
    return database.select().from(parentAiConsultations)
      .where(and(eq(parentAiConsultations.parentId, parentId), eq(parentAiConsultations.consultationType, consultationType)))
...
export async function getParentAiConsultation(consultationId: number) {
  const database = await getDb();
  if (!database) return null;
  const rows = await database.select().from(parentAiConsultations)
    .where(eq(parentAiConsultations.id, consultationId));
  return rows[0] || null;
}
export async function updateParentAiConsultation(consultationId: number, data: Partial<InsertParentAiConsultation>) {
  const database = await getDb();
  if (!database) return;
  await database.update(parentAiConsultations).set(data).where(eq(parentAiConsultations.id, consultationId));
}

export async function deleteParentAiConsultation(consultationId: number) {
  const database = await getDb();
  if (!database) return;
  await database.delete(parentAiConsultations).where(eq(parentAiConsultations.id, consultationId));
}
