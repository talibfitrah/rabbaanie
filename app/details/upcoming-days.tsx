...
function getDetailedDays(now: Date, lang: Lang): DayDetail[] {
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    if (fH.month===12 && fH.day>=1 && fH.day<=8) {
      events.push({ name: tx(lang,`${fH.day}e Dhul-Hijjah`,`${fH.day}th Dhul-Hijjah`,`${fH.day} ذو الحجة`), detail: tx(lang,"Goede daden geliefder bij Allaah dan op enige andere dag","Good deeds more beloved to Allaah than any other day","العمل الصالح أحب إلى الله من أي يوم آخر"), fasting: "recommended", evidence: tx(lang, "«Maa min ayyaamin al-'amalu as-saalihu feehinna ahabbu ilaa Allaah...» — Bukhaari", "«There are no days in which righteous deeds are more beloved to Allaah...» — al-Bukhaari", "«ما من أيام العمل الصالح فيهن أحب إلى الله من هذه الأيام العشر» — البخاري"), parentAction: tx(lang,"Vermeerder dhikr, sadaqah en takbier als gezin","Increase dhikr, sadaqah and takbeer as a family","أكثروا من الذكر والصدقة والتكبير كعائلة"), preparation: tx(lang,"Takbier; goede daden; geen nagels knippen","Takbeer; good deeds; no cutting nails","تكبير؛ أعمال صالحة؛ لا تقص الأظافر") });
    }
