...
function renderFormattedText(text: string, colors: any, isRTL: boolean) {
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith("##") || trimmed.startsWith("**")) {
      ...
      elements.push(
        <Text key={key++} style={{ fontSize: 15, fontWeight: "700", color: colors.foreground, marginTop: 12, marginBottom: 4, textAlign: isRTL ? "right" : "left", writingDirection: isRTL ? "rtl" : "ltr" }}>
          {headerText}
        </Text>
      );
      ...
      elements.push(
        <View key={key++} style={{ flexDirection: isRTL ? "row-reverse" : "row", alignItems: "flex-start", gap: 8, marginBottom: 4, paddingHorizontal: 4 }}>
          ...
          <Text style={{ flex: 1, fontSize: 13, color: colors.foreground, lineHeight: 21, textAlign: isRTL ? "right" : "left", writingDirection: isRTL ? "rtl" : "ltr" }}>{bulletText}</Text>
        </View>
      );
      ...
      elements.push(
        <View key={key++} style={{ flexDirection: isRTL ? "row-reverse" : "row", alignItems: "flex-start", gap: 8, marginBottom: 4, paddingHorizontal: 4 }}>
          ...
          <Text style={{ flex: 1, fontSize: 13, color: colors.foreground, lineHeight: 21, textAlign: isRTL ? "right" : "left", writingDirection: isRTL ? "rtl" : "ltr" }}>{numText}</Text>
        </View>
      );
    } else {
      const cleanText = trimmed.replace(/\*\*/g, "");
      elements.push(
        <Text key={key++} style={{ fontSize: 13, color: colors.foreground, lineHeight: 21, textAlign: isRTL ? "right" : "left", writingDirection: isRTL ? "rtl" : "ltr", marginBottom: 4 }}>
          {cleanText}
        </Text>
      );
    }
...
export default function PersonalAdviceScreen() {
  ...
  // Fetch quick tips from AI on mount and when relevant state changes
  useEffect(() => {
    if (state.parentProfileCompleted) fetchQuickTips();
  }, [language, state.parentProfile, state.children?.length]);
  ...
  async function fetchQuickTips() {
    ...
    try {
      ...
      if (cached) {
        const { tips, date, sig } = JSON.parse(cached);
        const today = currentWeekKey();
        if (date === today && sig === adviceDiagnosticSig(state) && tips && tips.length > 0) {
          setQuickTips(tips);
    ...
    try {
      ...
      const unresolvedIssues = (state.issues || []).filter((i: any) => !i.resolved).map((i: any) => i.description).slice(0, 5);
      ...
      const response = await fetch(`${baseUrl}/api/advice/quicktips`, {
        ...
        body: JSON.stringify({
          ...
          season,
          language,
          dailyCheckin: state.dailyCheckins?.find((c: any) => c.date === now.toISOString().slice(0, 10)) || null,
          recentCheckins: (state.dailyCheckins || []).slice(-7),
          unresolvedIssues,
          childrenEnvironments: (state.children || []).map((c: any) => {
            const env = (state.environments || []).find((e: any) => e.childId === c.id);
            return {
              childName: c.name,
              childAge: c.birthDate ? calculateExactAge(c.birthDate, lang) : undefined,
      ...
      if (data.tips && Array.isArray(data.tips) && data.tips.length > 0) {
        ...
        const today = currentWeekKey();
        await AsyncStorage.setItem(`quick_tips_cache_${language}`, JSON.stringify({ tips: data.tips, date: today, sig: adviceDiagnosticSig(state) }));
      }
    } catch (e) {
      console.error("[PersonalAdvice] fetchQuickTips error:", e);
      // Keep existing tips or show fallback
      if (quickTips.length === 0) {
        setQuickTips([tx(lang, "Maak du'aa voor uw gezin", "Make du'aa for your family", "ادعُ لعائلتك")]);
      }
    } finally {
      setQuickTipsLoading(false);
    }
        ...
        {quickTipsLoading ? (
          <View style={{ paddingVertical: 12, alignItems: "center" }}>
            ...
            <Text style={{ fontSize: 12, color: colors.muted, marginTop: 6 }}>
              {tx(lang, "Tips worden geladen...", "Loading tips...", "جارٍ تحميل النصائح...")}
            </Text>
          </View>
        ) : quickTips.map((adv: string, i: number) => (
