...
export default function FitrahScreen() {
        ...
        {fitrahGroups.map((group, index) => (
          ...
          >
            <View style={{ flexDirection: isRTL ? "row-reverse" : "row", alignItems: "center", gap: 12 }}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ color: colors.foreground, fontSize: 16, fontWeight: "700", textAlign: "right", writingDirection: "rtl" }}>{getText(group.title, lang)}</Text>
                <Text style={{ color: colors.primary, fontSize: 12, fontWeight: "600", marginTop: 2, textAlign: "right", writingDirection: "rtl" }}>{getText(group.subtitle, lang)}</Text>
      ...
      {currentFitrah?.heartActions.map((action: any) => (
        ...
        >
          <View style={{ padding: 12, flexDirection: isRTL ? "row-reverse" : "row", alignItems: "center", justifyContent: "space-between" }}>
            <View style={{ flex: 1 }}>
              <Text style={{ color: "#6A1B9A", fontSize: 14, fontWeight: "700", textAlign: "right", writingDirection: "rtl" }}>{getText(action.title, lang)}</Text>
              <Text style={{ color: colors.muted, fontSize: 11, marginTop: 2, textAlign: "right", writingDirection: "rtl" }} numberOfLines={expandedAction === action.id ? undefined : 1}>{getText(action.description, lang)}</Text>
            </View>
            <Pressable
            ...
            <View style={{ paddingHorizontal: 12, paddingBottom: 12, borderTopWidth: 0.5, borderTopColor: colors.border, paddingTop: 10 }}>
              {action.method ? (
                <View style={{ marginBottom: 8 }}>
                  <Text style={{ color: "#2E7D32", fontSize: 11, fontWeight: "700", marginBottom: 3, textAlign: "right", writingDirection: "rtl" }}>
                    {tx(lang, "METHODE:", "METHOD:", "الطريقة:")}
                  </Text>
                  <Text style={{ color: colors.foreground, fontSize: 12, lineHeight: 20, textAlign: "right", writingDirection: "rtl" }}>{getText(action.method, lang)}</Text>
                </View>
              ) : null}
              {action.daleel ? (
                <View style={{ backgroundColor: colors.background, borderRadius: 8, padding: 8 }}>
                  <Text style={{ color: colors.primary, fontSize: 11, fontWeight: "700", marginBottom: 3, textAlign: "right", writingDirection: "rtl" }}>
                    {tx(lang, "BEWIJS:", "EVIDENCE:", "الدليل:")}
                  </Text>
                  <Text style={{ color: colors.primary, fontSize: 11, lineHeight: 20, fontStyle: "italic", textAlign: "right", writingDirection: "rtl" }}>﴿ {getText(action.daleel, lang)} ﴾</Text>
                  {action.sources && action.sources.length > 0 && (
                    <View style={{ marginTop: 6, paddingTop: 6, borderTopWidth: 0.5, borderTopColor: colors.border }}>
                      <Text style={{ fontSize: 10, fontWeight: "600", color: colors.muted, marginBottom: 2, textAlign: "right", writingDirection: "rtl" }}>
                        {tx(lang, "Bronvermelding:", "Source:", "التخريج:")}
                      </Text>
                      {action.sources.map((src: any, si: number) => (
                        <Text key={si} style={{ fontSize: 10, color: colors.muted, lineHeight: 16, textAlign: "right", writingDirection: "rtl" }}>
                          {src.type === "quran" ? "📖 " : "📜 "}
            ...
            return (
              ...
              >
                <View style={{ padding: 14, flexDirection: isRTL ? "row-reverse" : "row", alignItems: "center", justifyContent: "space-between" }}>
                  <View style={{ flex: 1 }}>
                    <Text style={{ fontSize: 17, fontWeight: "700", color: "#E65100", textAlign: "right", writingDirection: "rtl" }}>{item.name}</Text>
                    {item.reason && (
                      <Text style={{ fontSize: 11, color: colors.muted, marginTop: 3, textAlign: "right", writingDirection: "rtl", lineHeight: 18 }}>{getTranslatedText(item.name, "reason", item.reason)}</Text>
                    )}
                  </View>
                  ...
                  <View style={{ paddingHorizontal: 14, paddingBottom: 14, borderTopWidth: 0.5, borderTopColor: colors.border, paddingTop: 12 }}>
                    ...
                    {item.meaning && (
                      <View style={{ marginBottom: 10 }}>
                        <Text style={{ fontSize: 12, fontWeight: "700", color: colors.primary, marginBottom: 4, textAlign: "right", writingDirection: "rtl" }}>
                          {tx(lang, "Betekenis:", "Meaning:", "المعنى:")}
                        </Text>
                        <Text style={{ fontSize: 12, color: colors.foreground, lineHeight: 22, textAlign: "right", writingDirection: "rtl" }}>{getTranslatedText(item.name, "meaning", item.meaning)}</Text>
                      </View>
                    )}
                    {/* الشرح */}
                    {item.explanation && (
                      <View style={{ marginBottom: 10 }}>
                        <Text style={{ fontSize: 12, fontWeight: "700", color: "#455A64", marginBottom: 4, textAlign: "right", writingDirection: "rtl" }}>
                          {tx(lang, "Uitleg:", "Explanation:", "الشرح:")}
                        </Text>
                        <Text style={{ fontSize: 12, color: colors.foreground, lineHeight: 22, textAlign: "right", writingDirection: "rtl" }}>{getTranslatedText(item.name, "explanation", item.explanation)}</Text>
                      </View>
                    )}
                    {/* الأدلة */}
                    {item.evidence && (
                      <View style={{ marginBottom: 10, backgroundColor: colors.background, borderRadius: 8, padding: 10 }}>
                        ...
                        <Text style={{ fontSize: 11, color: colors.primary, lineHeight: 20, fontStyle: "italic", textAlign: "right", writingDirection: "rtl" }}>{getTranslatedText(item.name, "evidence", item.evidence)}</Text>
                      </View>
                    )}
                    {/* الترغيب والترهيب */}
                    {item.targhib_tarhib && (
                      <View style={{ marginBottom: 10 }}>
                        <Text style={{ fontSize: 12, fontWeight: "700", color: "#C62828", marginBottom: 4, textAlign: "right", writingDirection: "rtl" }}>
                          {tx(lang, "Aanmoediging & waarschuwing:", "Encouragement & warning:", "الترغيب والترهيب:")}
                        </Text>
                        <Text style={{ fontSize: 12, color: colors.foreground, lineHeight: 22, textAlign: "right", writingDirection: "rtl" }}>{getTranslatedText(item.name, "targhib_tarhib", item.targhib_tarhib)}</Text>
                      </View>
                    )}
                    {/* تصفية */}
                    {item.tasfiya && item.tasfiya.length > 0 && (
                      <View style={{ marginBottom: 10 }}>
                        <Text style={{ fontSize: 12, fontWeight: "700", color: "#2E7D32", marginBottom: 4, textAlign: "right", writingDirection: "rtl" }}>
                          {tx(lang, "Tasfiya (wat planten we?):", "Tasfiya (what do we plant?):", "التصفية (ماذا نغرس؟):")}
                        </Text>
                        {item.tasfiya.map((t: any, i: number) => (
                          <Text key={i} style={{ fontSize: 12, color: colors.foreground, lineHeight: 22, textAlign: "right", writingDirection: "rtl", paddingRight: isRTL ? 10 : 0, paddingLeft: isRTL ? 0 : 10 }}>
                    ...
                    {item.tazkiya && item.tazkiya.length > 0 && (
                      <View style={{ marginBottom: 10 }}>
                        <Text style={{ fontSize: 12, fontWeight: "700", color: "#1565C0", marginBottom: 4, textAlign: "right", writingDirection: "rtl" }}>
                    ...
                    {item.tarbiya && item.tarbiya.length > 0 && (
                      <View style={{ marginBottom: 10 }}>
                        <Text style={{ fontSize: 12, fontWeight: "700", color: "#E65100", marginBottom: 4, textAlign: "right", writingDirection: "rtl" }}>
                    ...
                    {item.tarbiya_jawarih && item.tarbiya_jawarih.length > 0 && (
                      <View style={{ marginBottom: 10 }}>
                        <Text style={{ fontSize: 12, fontWeight: "700", color: "#4E342E", marginBottom: 4, textAlign: "right", writingDirection: "rtl" }}>
                    ...
                    {item.examples && item.examples.length > 0 && (
                      <View style={{ backgroundColor: colors.background, borderRadius: 8, padding: 10 }}>
                        <Text style={{ fontSize: 12, fontWeight: "700", color: "#6A1B9A", marginBottom: 4, textAlign: "right", writingDirection: "rtl" }}>
                    ...
                    {item.howToPresent && (
                      <View style={{ marginTop: 8 }}>
                        <Text style={{ fontSize: 12, fontWeight: "700", color: "#E65100", marginBottom: 3, textAlign: "right", writingDirection: "rtl" }}>
                          {tx(lang, "Presentatie:", "Presentation:", "طريقة التقديم:")}
                        </Text>
                        <Text style={{ fontSize: 12, color: colors.foreground, lineHeight: 20, textAlign: "right", writingDirection: "rtl" }}>{getTranslatedText(item.name, "howToPresent", item.howToPresent)}</Text>
                      </View>
                    )}
                  </View>
                )}
          ...
          {currentNames.groups.map((group: any) => {
            ...
            return (
              ...
              >
                <View style={{ padding: 14, flexDirection: isRTL ? "row-reverse" : "row", alignItems: "center", justifyContent: "space-between" }}>
                  <View style={{ flex: 1 }}>
                    <Text style={{ fontSize: 15, fontWeight: "700", color: "#E65100", textAlign: "right", writingDirection: "rtl" }}>{getText(group.group, lang)}</Text>
                    <Text style={{ fontSize: 11, color: colors.muted, marginTop: 3, textAlign: "right", writingDirection: "rtl" }}>
                      {nameStrings.join(" • ")}
                    </Text>
                  </View>
                  <Text style={{ fontSize: 12, color: colors.muted }}>{isExpanded ? "▲" : "▼"}</Text>
                ...
                {isExpanded && (
                  <View style={{ paddingHorizontal: 14, paddingBottom: 14, borderTopWidth: 0.5, borderTopColor: colors.border, paddingTop: 10 }}>
                    {group.buildsOn && (
                      <Text style={{ fontSize: 11, color: colors.muted, textAlign: "right", writingDirection: "rtl", marginBottom: 4 }}>
                        {tx(lang, "Bouwt voort op:", "Builds on:", "يبني على:")} {getText(group.buildsOn, lang)}
                      </Text>
                    )}
                    {group.preparesFor && (
                      <Text style={{ fontSize: 11, color: "#E65100", textAlign: "right", writingDirection: "rtl", lineHeight: 18, marginBottom: 10 }}>
                      ...
                      return (
                        ...
                        >
                          <View style={{ padding: 10, flexDirection: isRTL ? "row-reverse" : "row", alignItems: "center", justifyContent: "space-between" }}>
                            <Text style={{ fontSize: 15, fontWeight: "700", color: "#E65100", textAlign: "right", writingDirection: "rtl" }}>{nameKey}</Text>
                          ...
                          {isNameExpanded && (
                            <View style={{ paddingHorizontal: 10, paddingBottom: 12, borderTopWidth: 0.5, borderTopColor: colors.border, paddingTop: 8 }}>
                              ...
                              {nameItem.tasfiya && nameItem.tasfiya.length > 0 && (
                                <View style={{ marginBottom: 8 }}>
                                  <Text style={{ fontSize: 11, fontWeight: "700", color: "#2E7D32", marginBottom: 3, textAlign: "right", writingDirection: "rtl" }}>
                                    {tx(lang, "Tasfiya:", "Tasfiya:", "التصفية:")}
                                  </Text>
                                  {nameItem.tasfiya.map((t: any, i: number) => (
                                    <Text key={i} style={{ fontSize: 11, color: colors.foreground, lineHeight: 20, textAlign: "right", writingDirection: "rtl", paddingRight: isRTL ? 8 : 0, paddingLeft: isRTL ? 0 : 8 }}>
                              ...
                              {nameItem.tazkiya && nameItem.tazkiya.length > 0 && (
                                <View style={{ marginBottom: 8 }}>
                                  <Text style={{ fontSize: 11, fontWeight: "700", color: "#1565C0", marginBottom: 3, textAlign: "right", writingDirection: "rtl" }}>
                              ...
                              {nameItem.tarbiya && nameItem.tarbiya.length > 0 && (
                                <View style={{ marginBottom: 8 }}>
                                  <Text style={{ fontSize: 11, fontWeight: "700", color: "#E65100", marginBottom: 3, textAlign: "right", writingDirection: "rtl" }}>
                              ...
                              {nameItem.examples && nameItem.examples.length > 0 && (
                                <View style={{ backgroundColor: colors.surface, borderRadius: 8, padding: 8 }}>
                                  <Text style={{ fontSize: 11, fontWeight: "700", color: "#6A1B9A", marginBottom: 3, textAlign: "right", writingDirection: "rtl" }}>
        ...
        <View style={{ gap: 8 }}>
          ...
          {currentNames.practicalExample && (
            <View style={{ backgroundColor: "#FFF8E1", borderRadius: 12, padding: 14, borderWidth: 1, borderColor: "#FFD54F", marginBottom: 4 }}>
              <Text style={{ fontSize: 14, fontWeight: "700", color: "#E65100", textAlign: "right", writingDirection: "rtl", marginBottom: 6 }}>
              ...
              {currentNames.practicalExample.ageSpecific?.tasfiya && (
                <View style={{ marginBottom: 6 }}>
                  <Text style={{ fontSize: 11, fontWeight: "600", color: "#2E7D32", textAlign: "right", writingDirection: "rtl" }}>{tx(lang, "Tasfiya:", "Tasfiya:", "التصفية:")}</Text>
                  {currentNames.practicalExample.ageSpecific.tasfiya.map((t: any, i: number) => (
                    <Text key={i} style={{ fontSize: 11, color: colors.foreground, lineHeight: 18, textAlign: "right", writingDirection: "rtl" }}>• {getText(t, lang)}</Text>
                  ))}
                </View>
              )}
              {currentNames.practicalExample.ageSpecific?.tarbiya && (
                <View style={{ marginBottom: 6 }}>
                  <Text style={{ fontSize: 11, fontWeight: "600", color: "#E65100", textAlign: "right", writingDirection: "rtl" }}>{tx(lang, "Tarbiya:", "Tarbiya:", "التربية:")}</Text>
          ...
          {currentNames.heartStations.map((station: any) => {
            ...
            return (
              ...
              >
                <View style={{ padding: 14, flexDirection: isRTL ? "row-reverse" : "row", alignItems: "center", justifyContent: "space-between" }}>
                  <View style={{ flex: 1 }}>
                    <Text style={{ fontSize: 15, fontWeight: "700", color: "#E65100", textAlign: "right", writingDirection: "rtl" }}>{getTranslatedText(stationKey, "station", station.station)}</Text>
                    {station.meaning && (
                      <Text style={{ fontSize: 11, color: colors.muted, marginTop: 3, textAlign: "right", writingDirection: "rtl", lineHeight: 18 }}>{getTranslatedText(stationKey, "meaning", station.meaning)}</Text>
                    )}
                  </View>
                  <Text style={{ fontSize: 12, color: colors.muted }}>{isExpanded ? "▲" : "▼"}</Text>
                </View>
                {isExpanded && (
                  <View style={{ paddingHorizontal: 14, paddingBottom: 14, borderTopWidth: 0.5, borderTopColor: colors.border, paddingTop: 10 }}>
                    ...
                    {station.method && (
                      <View style={{ marginBottom: 8 }}>
                        <Text style={{ fontSize: 12, fontWeight: "600", color: "#2E7D32", textAlign: "right", writingDirection: "rtl" }}>{tx(lang, "Methode:", "Method:", "الأسلوب:")}</Text>
                        <Text style={{ fontSize: 12, color: colors.foreground, lineHeight: 20, textAlign: "right", writingDirection: "rtl" }}>{getTranslatedText(stationKey, "method", station.method)}</Text>
                      </View>
                    )}
                    {station.evidence && (
                      <View style={{ backgroundColor: colors.background, borderRadius: 8, padding: 8 }}>
                        <Text style={{ fontSize: 12, color: colors.primary, lineHeight: 20, textAlign: "right", writingDirection: "rtl", fontStyle: "italic" }}>
        ...
        return (
          ...
          >
            <View style={{ padding: 12, flexDirection: isRTL ? "row-reverse" : "row", alignItems: "center" }}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ color: colors.foreground, fontSize: 14, fontWeight: "700", textAlign: "right", writingDirection: "rtl", lineHeight: 22 }} numberOfLines={isExpanded ? undefined : 2}>
            ...
            {isExpanded && (
              <View style={{ paddingHorizontal: 12, paddingBottom: 12, borderTopWidth: 0.5, borderTopColor: colors.border, paddingTop: 10 }}>
                {/* Description */}
                <Text style={{ color: colors.foreground, fontSize: 12, lineHeight: 22, textAlign: "right", writingDirection: "rtl", marginBottom: 10 }}>
                ...
                {getConceptSource(item, lang) ? (
                  <View style={{ backgroundColor: "#FFFBEB", borderRadius: 8, padding: 10, marginBottom: 8 }}>
                    <Text style={{ color: "#92400E", fontSize: 11, fontWeight: "700", marginBottom: 3, textAlign: "right", writingDirection: "rtl" }}>
                      {tx(lang, "BEWIJS:", "EVIDENCE:", "الدليل:")}
                    </Text>
                    <Text style={{ color: "#78350F", fontSize: 12, lineHeight: 20, textAlign: "right", writingDirection: "rtl", fontStyle: "italic" }}>
                      {getConceptSource(item, lang)}
                    </Text>
                  </View>
                ) : null}
                ...
                {getConceptScholar(item, lang) ? (
                  <View style={{ backgroundColor: "#F3E5F5", borderRadius: 8, padding: 10 }}>
                    <Text style={{ color: "#6A1B9A", fontSize: 11, fontWeight: "700", marginBottom: 3, textAlign: "right", writingDirection: "rtl" }}>
                      {tx(lang, "GELEERDE:", "SCHOLAR:", "العالِم:")}
                    </Text>
                    <Text style={{ color: "#4A148C", fontSize: 12, lineHeight: 20, textAlign: "right", writingDirection: "rtl" }}>
                      {getConceptScholar(item, lang)}
                    </Text>
                  </View>
                ) : null}
  ...
  const renderDetail = () => (
    <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: 40 }}>
      ...
      <Pressable onPress={goBack} style={({ pressed }) => [{ opacity: pressed ? 0.6 : 1, marginBottom: 10 }]}>
        <Text style={{ color: colors.primary, fontSize: 13, fontWeight: "600", textAlign: "right", writingDirection: "rtl" }}>
      ...
      <View style={{ backgroundColor: colors.primary + "10", borderRadius: 14, padding: 14, borderWidth: 1, borderColor: colors.primary + "25", marginBottom: 14 }}>
        <Text style={{ color: colors.primary, fontSize: 18, fontWeight: "800", textAlign: "right", writingDirection: "rtl" }}>{getText(currentFitrah?.title, lang)}</Text>
        <Text style={{ color: colors.foreground, fontSize: 12, fontWeight: "600", marginTop: 3, textAlign: "right", writingDirection: "rtl" }}>{getText(currentFitrah?.subtitle, lang)}</Text>
        <Text style={{ color: colors.muted, fontSize: 11, marginTop: 6, lineHeight: 18, textAlign: "right", writingDirection: "rtl" }}>{getText(currentFitrah?.description, lang)}</Text>
      </View>
  ...
  const renderMisconceptions = () => {
    ...
    return (
      <View style={{ gap: 16 }}>
        <Text style={{ fontSize: 14, color: colors.muted, textAlign: "right", writingDirection: "rtl", marginBottom: 4 }}>
        ...
        {AGE_GROUPS.filter(g => ageGrouped[g]?.length > 0).map((groupName) => (
          <View key={groupName} style={{ marginBottom: 12 }}>
            <View style={{ backgroundColor: "#FFEBEE", borderRadius: 10, padding: 10, marginBottom: 8 }}>
              <Text style={{ fontSize: 14, fontWeight: "700", color: "#B71C1C", textAlign: "right", writingDirection: "rtl" }}>
            ...
            {ageGrouped[groupName].map((item: any) => {
              ...
              return (
                ...
                >
                  <Text style={{ fontSize: 14, fontWeight: "600", color: "#B71C1C", textAlign: "right", writingDirection: "rtl", lineHeight: 22 }}>
                    {misconceptionText}
                  </Text>
                  {isExpanded && (
                    <View style={{ marginTop: 12, gap: 12 }}>
                      {clarificationText ? (
                        <View style={{ backgroundColor: "#FFF8E1", borderRadius: 8, padding: 10 }}>
                          <Text style={{ fontSize: 12, fontWeight: "700", color: "#F57F17", marginBottom: 4, textAlign: "right", writingDirection: "rtl" }}>
                            {tx(lang, "Hun bewering", "Their claim", "زعمهم")}
                          </Text>
                          <Text style={{ fontSize: 13, color: "#333", textAlign: "right", writingDirection: "rtl", lineHeight: 20 }}>{clarificationText}</Text>
                        </View>
                      ) : null}
                      {refutationText ? (
                        <View style={{ backgroundColor: "#E8F5E9", borderRadius: 8, padding: 10 }}>
                          ...
                          <Text style={{ fontSize: 13, color: "#333", textAlign: "right", writingDirection: "rtl", lineHeight: 20 }}>{refutationText}</Text>
                        </View>
                      ) : null}
                      {evidencesText ? (
                        <View style={{ backgroundColor: "#E3F2FD", borderRadius: 8, padding: 10 }}>
                          ...
                          <Text style={{ fontSize: 13, color: "#333", textAlign: "right", writingDirection: "rtl", lineHeight: 20 }}>{evidencesText}</Text>
                        </View>
                      ) : null}
                      {practicalText ? (
                        <View style={{ backgroundColor: "#F3E5F5", borderRadius: 8, padding: 10 }}>
                          ...
                          <Text style={{ fontSize: 13, color: "#333", textAlign: "right", writingDirection: "rtl", lineHeight: 20 }}>{practicalText}</Text>
                        </View>
                      ) : null}
                    </View>
                  )}
        ...
        {Object.entries(topicGrouped).map(([groupName, items]) => (
          <View key={groupName} style={{ marginBottom: 12 }}>
            <View style={{ backgroundColor: "#FFF3E0", borderRadius: 10, padding: 10, marginBottom: 8 }}>
              <Text style={{ fontSize: 14, fontWeight: "700", color: "#E65100", textAlign: "right", writingDirection: "rtl" }}>
                    ...
                    <View style={{ marginTop: 12, gap: 12 }}>
                      ...
                      {clarificationText ? (
                        <View style={{ backgroundColor: "#FFF8E1", borderRadius: 8, padding: 10 }}>
                          ...
                          <Text style={{ fontSize: 13, color: "#374151", lineHeight: 20, textAlign: "right", writingDirection: "rtl" }}>
                            {clarificationText}
                          </Text>
                        </View>
                      ) : null}
        ...
        {favItems.map((fav, idx) => (
          <View key={idx} style={{ backgroundColor: colors.surface, borderRadius: 10, borderWidth: 1, borderColor: colors.border, padding: 12 }}>
            <View style={{ flexDirection: isRTL ? "row-reverse" : "row", alignItems: "center", gap: 8 }}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ color: colors.foreground, fontSize: 13, fontWeight: "600", textAlign: "right", writingDirection: "rtl", lineHeight: 20 }} numberOfLines={2}>
                ...
                <Text style={{ color: colors.muted, fontSize: 10, marginTop: 2, textAlign: "right", writingDirection: "rtl" }}>
                  {getText(fitrahGroups[fav.groupIndex]?.title, lang)}
                </Text>
                {(fav.type === "trait" && fav.item.method) && (
                  <Text style={{ color: colors.muted, fontSize: 11, marginTop: 4, textAlign: "right", writingDirection: "rtl", lineHeight: 18 }} numberOfLines={3}>
  ...
  const renderSearchResults = () => {
    ...
    return (
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ paddingBottom: 40, gap: 8 }}>
        <Text style={{ color: colors.muted, fontSize: 11, textAlign: "right", writingDirection: "rtl", marginBottom: 4 }}>
    ...
    <ScreenContainer className="px-4 pt-4">
      ...
      <View style={{ marginBottom: 12 }}>
        <View style={{ flexDirection: isRTL ? "row-reverse" : "row", alignItems: "center", justifyContent: "space-between" }}>
          <Text style={{ color: colors.foreground, fontSize: 20, fontWeight: "800", textAlign: "right", writingDirection: "rtl" }}>
        ...
        <Text style={{ color: colors.muted, fontSize: 11, marginTop: 3, textAlign: "right", writingDirection: "rtl", lineHeight: 18 }}>
