...
export default function TreatmentsScreen() {
  ...
  if (!state.parentProfileCompleted) {
    return (
      ...
      >
        <View className="rounded-2xl p-6" style={{ backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.primary + "40" }}>
          <Text className="text-xl font-bold mb-3" style={{ color: colors.foreground }}>
            {t("treatments.fill_profile")}
          </Text>
          <Text className="text-sm leading-5 mb-6" style={{ color: colors.muted }}>
          ...
          >
            <Text className="text-white text-base font-bold">{t("treatments.go_profile")}</Text>
          </Pressable>
        </View>
      </View>
    );
  ...
  return (
    <View className="flex-1" style={{ backgroundColor: colors.background }}>
      ...
      >
        ...
        <View style={{ flex: 1 }}>
          <Text style={{ color: colors.foreground, fontSize: 14, fontWeight: "600" }}>
            {language === "ar" ? "\u0645\u0642\u0627\u0644\u0627\u062a \u0648\u0646\u0635\u0627\u0626\u062d" : language === "en" ? "Articles & Tips" : "Artikelen & Tips"}
          </Text>
          <Text style={{ color: colors.muted, fontSize: 12 }}>
            {language === "ar" ? "\u0627\u0637\u0644\u0639 \u0639\u0644\u0649 \u0627\u0644\u0645\u062d\u062a\u0648\u0649 \u0627\u0644\u0645\u062a\u0639\u0644\u0642 \u0628\u0627\u0644\u0639\u0644\u0627\u062c\u0627\u062a" : language === "en" ? "View content related to treatments" : "Bekijk content over behandelingen"}
          </Text>
        </View>
          ...
          <View key={child.id} className="mb-6">
            <View className="flex-row items-center justify-between mb-3">
              ...
              <Pressable
                ...
                style={({ pressed }) => [
                  { backgroundColor: colors.error + "15", borderRadius: 8, paddingHorizontal: 12, paddingVertical: 6, opacity: pressed ? 0.7 : 1 },
                ]}
              >
            ...
            {childOpenIssues.length === 0 && (
              <View
                ...
                style={{ backgroundColor: colors.success + "10", borderWidth: 1, borderColor: colors.success + "30" }}
              >
                <Text className="text-sm" style={{ color: colors.success }}>
                  {(() => { const n = child.name; const m = n.match(/^(Kind|Child|طفل)\s*(\d+)$/i); const dn = m ? (language === "ar" ? `طفل ${m[2]}` : language === "en" ? `Child ${m[2]}` : `Kind ${m[2]}`) : n; return language === "ar" ? `لا توجد مشاكل مفتوحة لـ ${dn}` : language === "en" ? `No open issues for ${dn}` : `Geen openstaande issues voor ${dn}`; })()}
                </Text>
              </View>
            )}
            ...
            {childOpenIssues.map((issue) => (
              ...
              >
                ...
                {issue.treatmentPlan && (
                  <View className="mt-2 rounded-lg p-3" style={{ backgroundColor: colors.primary + "08" }}>
                    <Text className="text-xs font-bold mb-1" style={{ color: colors.primary }}>
                      {t("treatments.plan")}:
                    </Text>
                    <Text className="text-xs leading-4" style={{ color: colors.foreground }}>
                      {issue.treatmentPlan}
                    </Text>
                  </View>
                )}
                ...
                <Pressable
                  onPress={() => router.push(`/child/${child.id}`)}
                  style={({ pressed }) => [
                    { backgroundColor: colors.primary, borderRadius: 8, paddingVertical: 10, alignItems: "center", marginTop: 12, opacity: pressed ? 0.8 : 1 },
                  ]}
                >
      ...
      {resolvedIssues.length > 0 && (
        <View className="mt-4">
          <Text className="text-base font-bold mb-3" style={{ color: colors.muted }}>
          ...
          {resolvedIssues.map((issue) => {
            ...
            return (
              <View
                ...
                style={{ backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.success + "30" }}
              >
                <View className="flex-row items-center gap-2">
                  <View className="rounded-full px-2 py-0.5" style={{ backgroundColor: colors.success + "15" }}>
                    <Text className="text-xs font-medium" style={{ color: colors.success }}>{t("treatments.resolved")}</Text>
                  </View>
                  <Text className="text-xs" style={{ color: colors.muted }}>{child?.name}</Text>
                </View>
