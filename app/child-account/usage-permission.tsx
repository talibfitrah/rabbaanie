...
import { Text, View, ScrollView, TouchableOpacity, Platform, Alert, AppState } from "react-native";
...
export default function UsagePermissionScreen() {
  ...
  // When permission is granted, start fetching data
  useEffect(() => {
    if (permissionGranted) {
      fetchAndStoreExternalUsage().catch(console.error);
    }
  }, [permissionGranted]);
    ...
    return (
      <ScreenContainer className="p-6" edges={["top", "bottom", "left", "right"]}>
        <View className="flex-1 items-center justify-center gap-4">
          ...
          <TouchableOpacity
            onPress={() => router.back()}
            style={{ backgroundColor: colors.primary, paddingHorizontal: 32, paddingVertical: 14, borderRadius: 12, marginTop: 20 }}
          >
            <Text style={{ color: "#fff", fontWeight: "700", fontSize: 16 }}>
              {language === "ar" ? "رجوع" : language === "nl" ? "Terug" : "Back"}
            </Text>
          </TouchableOpacity>
        </View>
      </ScreenContainer>
  ...
  if (!moduleAvailable && !checking) {
    return (
      <ScreenContainer className="p-6" edges={["top", "bottom", "left", "right"]}>
        <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: "center", alignItems: "center", gap: 16 }}>
          <Text style={{ fontSize: 60 }}>🔧</Text>
          <Text className="text-xl font-bold text-foreground text-center">
            {language === "ar"
          ...
          <Text className="text-base text-muted text-center mt-2" style={{ paddingHorizontal: 20 }}>
            {language === "ar"
          ...
          <View style={{ backgroundColor: colors.surface, borderRadius: 12, padding: 16, marginTop: 16, width: "100%" }}>
            <Text className="text-sm font-semibold text-foreground mb-2" style={{ textAlign: isRTL ? "right" : "left" }}>
              {language === "ar" ? "📋 خطوات التفعيل:" : language === "nl" ? "📋 Activeringsstappen:" : "📋 Activation steps:"}
            </Text>
            {[
              language === "ar" ? "1. انشر التطبيق كـ APK (زر Publish)" : language === "nl" ? "1. Publiceer de app als APK (Publish-knop)" : "1. Publish the app as APK (Publish button)",
              language === "ar" ? "2. ثبّت APK على هاتف الطفل" : language === "nl" ? "2. Installeer APK op het telefoon van het kind" : "2. Install APK on the child's phone",
              language === "ar" ? "3. افتح هذه الشاشة مرة أخرى" : language === "nl" ? "3. Open dit scherm opnieuw" : "3. Open this screen again",
              language === "ar" ? "4. امنح إذن الوصول لبيانات الاستخدام" : language === "nl" ? "4. Verleen toegang tot gebruiksgegevens" : "4. Grant usage data access permission",
            ].map((step, i) => (
              <Text key={i} className="text-sm text-muted" style={{ textAlign: isRTL ? "right" : "left", marginTop: 4 }}>
                {step}
              </Text>
            ))}
          </View>
          ...
          <TouchableOpacity
            onPress={() => router.back()}
            style={{ backgroundColor: colors.primary, paddingHorizontal: 32, paddingVertical: 14, borderRadius: 12, marginTop: 24 }}
          >
            <Text style={{ color: "#fff", fontWeight: "700", fontSize: 16 }}>
              {language === "ar" ? "فهمت" : language === "nl" ? "Begrepen" : "Got it"}
            </Text>
          </TouchableOpacity>
        </ScrollView>
      </ScreenContainer>
  ...
  // Permission granted - success state
  if (permissionGranted) {
    return (
      <ScreenContainer className="p-6" edges={["top", "bottom", "left", "right"]}>
        <View className="flex-1 items-center justify-center gap-4">
    ...
    <ScreenContainer className="p-6" edges={["top", "bottom", "left", "right"]}>
      <ScrollView contentContainerStyle={{ flexGrow: 1, gap: 16 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={{ backgroundColor: colors.surface, borderRadius: 12, padding: 16, marginTop: 8 }}>
          <Text className="text-base text-foreground" style={{ textAlign: isRTL ? "right" : "left", lineHeight: 24 }}>
            {language === "ar"
          ...
          <View style={{ marginTop: 12, gap: 8 }}>
            {[
              { icon: "📱", text: language === "ar" ? "معرفة التطبيقات المستخدمة يومياً" : language === "nl" ? "Dagelijks gebruikte apps zien" : "See daily used apps" },
              { icon: "⏱️", text: language === "ar" ? "حساب مدة استخدام كل تطبيق" : language === "nl" ? "Gebruiksduur per app berekenen" : "Calculate usage time per app" },
              { icon: "📊", text: language === "ar" ? "إرسال تقارير للوالدين" : language === "nl" ? "Rapporten naar ouders sturen" : "Send reports to parents" },
              { icon: "⚠️", text: language === "ar" ? "تنبيه عند الاستخدام المفرط" : language === "nl" ? "Waarschuwen bij overmatig gebruik" : "Alert on excessive usage" },
            ].map((item, i) => (
              <View key={i} style={{ flexDirection: isRTL ? "row-reverse" : "row", alignItems: "center", gap: 8 }}>
                <Text style={{ fontSize: 18 }}>{item.icon}</Text>
                <Text className="text-sm text-muted flex-1" style={{ textAlign: isRTL ? "right" : "left" }}>
                  {item.text}
                </Text>
              </View>
            ))}
        ...
        {/* Privacy note */}
        <View style={{ backgroundColor: "#FFF3E0", borderRadius: 12, padding: 12, borderWidth: 1, borderColor: "#FFE0B2" }}>
          <Text style={{ color: "#E65100", fontSize: 13, textAlign: isRTL ? "right" : "left", lineHeight: 20 }}>
            {language === "ar"
        ...
        <View style={{ backgroundColor: colors.surface, borderRadius: 12, padding: 16 }}>
          <Text className="text-sm font-semibold text-foreground mb-3" style={{ textAlign: isRTL ? "right" : "left" }}>
            {language === "ar" ? "خطوات التفعيل:" : language === "nl" ? "Stappen:" : "Steps:"}
          </Text>
          {[
            language === "ar" ? "اضغط على الزر أدناه لفتح الإعدادات" : language === "nl" ? "Druk op de knop hieronder om instellingen te openen" : "Press the button below to open settings",
            language === "ar" ? "ابحث عن تطبيق \"ربّاني\" في القائمة" : language === "nl" ? "Zoek de app \"Rabbaani\" in de lijst" : "Find the app \"Rabbaani\" in the list",
            language === "ar" ? "فعّل إذن الوصول لبيانات الاستخدام" : language === "nl" ? "Schakel toegang tot gebruiksgegevens in" : "Enable usage data access",
            language === "ar" ? "ارجع إلى التطبيق" : language === "nl" ? "Keer terug naar de app" : "Return to the app",
          ].map((step, i) => (
            <View key={i} style={{ flexDirection: isRTL ? "row-reverse" : "row", alignItems: "flex-start", gap: 8, marginTop: 8 }}>
              ...
              <Text className="text-sm text-muted flex-1" style={{ textAlign: isRTL ? "right" : "left", lineHeight: 20 }}>
                {step}
              </Text>
            </View>
          ))}
        ...
        >
          <Text className="text-sm text-muted">
            {language === "ar" ? "تخطي (يمكنك التفعيل لاحقاً)" : language === "nl" ? "Overslaan (later activeren)" : "Skip (activate later)"}
          </Text>
        </TouchableOpacity>
      </ScrollView>
    </ScreenContainer>
