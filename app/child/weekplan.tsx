...
function parsePlanIntoGroups(
  ...
  const parentPatterns = [
    ...
    /نصائح للوالد/i,
    /أولاً.*الوالد/i,
    /ADVIES VOOR DE OUDER/i,
    /ADVICE FOR THE PARENT/i,
    /مبادئ.*للوالد/i,
    /MINDSETS/i,
    /تحسين.*الصلة/i,
    /^\s*#?\s*\d+[\.\)]\s*نصيحة/i,
  ...
  for (const line of lines) {
    ...
    const isSubTitle =
      /^(?:\d+[\.\)]\s+|#{1,3}\s+)/.test(trimmed) && trimmed.length < 100;
  ...
  if (parentCards.length > 0) {
    groups.push({
      ...
      title: tx(
        lang,
        "De ouder met zichzelf",
        "The parent with themselves",
        "الوالد مع نفسه",
      ),
      color: "#166534",
      bgColor: "#F0FDF4",
  ...
  if (childCards.length > 0) {
    groups.push({
      ...
      title: tx(
        lang,
        `De ouder met ${childName}`,
        `The parent with ${childName}`,
        `الوالد مع ${childName}`,
      ),
      color: "#1E40AF",
      bgColor: "#EFF6FF",
...
export default function WeekplanScreen() {
  ...
  const groups = weekPlan
    ? parsePlanIntoGroups(weekPlan, lang, child.name)
    : [];
      ...
      <View style={s.header}>
        ...
        >
          {tx(
            lang,
            `Weekplan — ${child.name}`,
            `Week Plan — ${child.name}`,
            `الخطة الأسبوعية — ${child.name}`,
          )}
        </Text>
        {age && (
          <Text
          ...
          >
            {tx(
              ...
              `السنة ${yearKey ? yearKey.replace("Jaar ", "") : ""} — الأسبوع ${weekInYear}`,
            )}
          </Text>
        )}
      </View>
      ...
      {loading ? (
        <View style={s.loadingBox}>
          ...
          >
            {tx(
              lang,
              "Weekplan wordt gegenereerd...",
              "Generating week plan...",
              "جارٍ إعداد الخطة الأسبوعية...",
            )}
          </Text>
        </View>
      ) : groups.length > 0 ? (
        ...
        <View style={s.loadingBox}>
          <Text style={{ color: colors.muted }}>
            {tx(
              ...
              "First fill in the birth date.",
              "أدخل تاريخ الميلاد أولاً.",
            )}
          </Text>
        </View>
      )}
      ...
      {isExpanded && (
        <View style={[s.cardBody, { borderTopColor: groupColor + "15" }]}>
          {card.content.map((item, idx) => (
            ...
            >
              <View
                style={[s.itemBullet, { backgroundColor: groupColor + "40" }]}
              />
              <Text
                style={[
                  s.itemText,
