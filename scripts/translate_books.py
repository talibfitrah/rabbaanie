...
def translate_batch(titles, target_lang):
    ...
    try:
        resp = client.chat.completions.create(
            model="gpt-5-nano",
            messages=[
                {"role": "system", "content": f"Translate each numbered Arabic title to {lang_name}. Keep Islamic terms transliterated (Tawheed, Fitrah, Sabr, Ikhlas). Output ONLY numbered translations, one per line."},
...
def process_book(book_num):
    ...
    book_path = f"/home/ubuntu/opvoedadvies_apk/assets/data/library/book_{book_num}.json"
    ...
    for lang in ['nl', 'en']:
        ...
        for i in range(0, len(items), BATCH_SIZE):
            ...
            print(f"  [{lang.upper()}] {batch_num}/{total_batches}", flush=True)
            time.sleep(0.2)
...
if __name__ == "__main__":
    # Process specific book if arg given, otherwise all
    if len(sys.argv) > 1:
        ...
        print(f"Total: {total} items translated", flush=True)
