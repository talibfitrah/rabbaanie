...
Backend-first: the resend endpoint must be live on production before any client build that calls it ships, or resend 404s for early updaters. Deploy rabbaanie-api, verify `/auth/2fa/resend` live, then cut the app release.
